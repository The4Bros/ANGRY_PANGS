...
Harpoon::Harpoon(Application* app)
{
	...
	rect = { 8 * app->windowModule->scale, 168 * app->windowModule->scale, 9 * app->windowModule->scale, 9 * app->windowModule->scale };
...
void Harpoon::Shoot_Harpoon(SDL_Rect player_rect)
{
	...
	rect.y = player_rect.y + 32 * app->windowModule->scale;
...
void Harpoon::Update()
{
	if (rect.y <= 8 * app->windowModule->scale)
	{
		if (source_rect.x > 9)
		{
